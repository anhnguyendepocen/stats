...
\title{Repeated Measures ANOVA\\Introduction to Statistics Using R (Psychology 9041B)}
...
\section{One-way Repeated-Measures ANOVA}

First let's load in the sample data. It's ``stacked'' in the usual
way, where each row is a single observation, and columns code
variables --- in this case \texttt{treatment}, which has four levels,
and \texttt{subject}, which has 10 levels.
...
\subsection{demo: ignore subjects}

As a demonstration, let's run this \emph{as if} it were simply a
between-subjects ANOVA, in other words, ignoring the fact that
observations from the four levels of \texttt{treatment} come from the
same subjects:
...
In this case the F-test of the \texttt{treatment} effect is
significant at $p=0.001890$.

\subsection{demo: include subjects as if it were a two-factor
  between-subjects design}

Now as another demonstration, let's run this ANOVA again \emph{as if}
it were a two-factor design, in which we include \texttt{subject} as a
factor. In this case, note that we cannot include both the main
effects \emph{and} the interaction effect --- since there is only one
...
Note how the SS, df, and MS for the \texttt{treatment} effect are
identical to the previous case where the \texttt{subject} factor was
not included in the model. What's changed, is that the \emph{error
  term} (the term denoted \texttt{Residuals}) is now smaller. In other
words, in the first model (\texttt{am1}), the SS was $77.00$ --- this
represents the ``leftover'' variability in the dependent variable that
is unaccounted for by the \texttt{treatment} factor. In the
\texttt{am2} model, the \texttt{subject} factor has a SS of $48.4$,
which is now sliced out of the error term, leaving only $28.6$. In
other words, the \texttt{subject} factor has accounted for a chunk of
...
F-ratio for the \texttt{treatment} effect is larger (and hence the
p-value is smaller) --- since it is equal to the MS for
\texttt{treatment} divided by the MS for the error term.

Note that this decrease in MS error is not ``free'', in other words
there is a cost: we give up degrees of freedom (was $36$ in
\texttt{am1} but is now reduced to $27$ in \texttt{am2}). Lower
...
\texttt{treatment} effect. Now all other things are \emph{not} equal,
because the inclusion of \texttt{subjects} has lowered the error term
...
in the model \emph{worth it}, given that we have to ``pay'' a certain
number of degrees of freedom?

\subsection{Correct method (univariate approach)}
...
\texttt{+Error(subject/treatment)}. This notation tells R to slice out
of the model an additional error term corresponding to the variance
accounted for by subjects. The \texttt{subjects/treatment} notation
tells R that the \texttt{treatment} factor is the repeated-measures
factor over which subjects applies.

As you can see the output is essentially the same, just reorganized a
bit. The variance accounted for by \texttt{subjects} has not only been
removed from the model's overall error term, but has even been
physically separated in the output. So now, the effect of
\texttt{treatment} is significant at $p=3.06e-05$.

This is the correct method of running a single factor
repeated-measures ANOVA, using what is called a \emph{univariate
  approach}. That is to say, there is a single dependent variable
(called \texttt{dv} in our data frame \texttt{mydata}). We have been
doing univariate procedures all along in the course so far.

\subsection{Multivariate approach}
...
provides ways of testing the \emph{sphericity assumption} --- the
assumption of homogeneity of variance of differences between groups
--- as well as providing corrected versions of the F-test assuming the
...
represent levels of the \texttt{treatment} factor. This may be
...
We now form the multivariate model using the \texttt{lm()} function in
\texttt{R}:
...
The \texttt{~1} notation simply tells R that there are no
between-subjects factors here ... in other words, only fit the model
using intercepts. If you type \texttt{mlm1} to look at the model
...
Now we must set up a variable that defines the \emph{design} of our
...
Now we must load the \texttt{car} library into R (we only have to do
this once in a given session), because we are going to make use of the
\texttt{Anova()} function (note the capital \texttt{A}, this is
different than the \texttt{anova()} function). The \texttt{Anova()}
...
We now define a new anova model object \texttt{mlm1.aov} by a function
call to \texttt{Anova()}:
...
The first argument, \texttt{mlm1}, is our multivariate model defined
above. The second argument, \texttt{idata=data.frame(rfactor)} passes
information about the within-subjects variable, in other words how
many levels there are. The third argument, \texttt{idesign=~rfactor},
passes information about the within-subjects design, in other words
that the variable that \texttt{rfactor} describes is the
repeated-measures variable. The fourth argument, \texttt{type="III"},
instructs \texttt{Anova()} to calculate the ``Type-III'' sums of
...
squares. In the \texttt{summary()} command we specify
\texttt{multivariate=FALSE} because we want to suppress the portion of
the output of the \texttt{Anova()} function that is related to
...
\texttt{rfactor} here) is significant at $p=3.06e-05$. Then we see
...
\texttt{Pr(>F[GG])} and \texttt{Pr(>F[HF])}, respectively). See your
Maxwell and Delaney readings for details about how these corrections
are computed.

\subsection{Comparisons between individual means}
...
We can then compute a probability using the \texttt{pf()} function:
...
\subsubsection{If you're concerned about sphericity}
